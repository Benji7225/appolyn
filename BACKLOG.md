...
- **(A) Bloc « revenus & abonnements » façon dashboard RevenueCat** : MRR, ARR, abonnés actifs, essais en cours, taux de conversion essai→payant, churn, LTV, revenu par cohorte. **FAISABLE quasi tel quel** : le SDK iOS d'Appolyn capte déjà les achats via StoreKit 2 (cf page Clients / `/api/sdk/ingest`). On dérive ces métriques des events d'achat déjà ingérés → pas de nouvelle dépendance. Présentation = cartes KPI + courbes (réutiliser la grille Analytics drag-and-drop existante). **Honnête** : ne montrer que ce qui découle de vraies données captées ; tant qu'aucune app cliente n'a de trafic, le bloc est vide (état vide explicite), pas de chiffres inventés.
- **(B) Funnel d'onboarding façon PostHog** : pour chaque écran d'onboarding de l'app du dev, une **barre de % + le nombre de personnes** (ex. Welcome 89 / 100%, écran 2 = 82 / 92%, écran 3 = 62…), pour voir où les gens décrochent. UI = liste verticale d'étapes, barre horizontale + compteur à droite.
  - ⚠️ **Limite honnête sur le « 100% automatique » voulu par Benji** : le SDK ne peut PAS deviner tout seul les écrans d'onboarding d'une app quelconque. Pour avoir le funnel, le dev doit **instrumenter chaque écran** (1 ligne, ex. `Appolyn.onboardingStep("welcome")` à l'apparition de chaque page). Ce n'est donc pas « zéro code » pour un onboarding arbitraire — mais on peut le rendre **dead-simple** (1 helper, noms libres ; l'ordre/les pourcentages se calculent ensuite côté Appolyn automatiquement). À faire : helper SDK `onboardingStep(_:)` + ingestion event `onboarding_step` + endpoint d'agrégation funnel + le bloc UI. Le « ça se met à jour automatiquement quand le dev change son onboarding » MARCHE pour l'ordre/les chiffres une fois les étapes instrumentées ; un nouvel écran non instrumenté n'apparaîtra pas tant que le dev n'a pas ajouté la ligne. Le dire clairement dans la doc SDK.
