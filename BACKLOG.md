...
- **Stripe (abonnement Appolyn)** — ⚠️ **NOTE PÉRIMÉE CORRIGÉE (19/06)** : les clés Stripe sont DÉJÀ présentes (`.env` local + Vercel **production**, `sk_live_` + `whsec_` + publishable, déposées il y a ~6 j). Le code de facturation (checkout + webhook + portail + retention) est branché. **N'est donc PLUS bloqué côté clés.** Reste seulement : (a) créer les **produits/prix** côté Stripe (lookup keys) — faisable par Claude via le connecteur Stripe si Benji l'autorise dans le navigateur ; (b) compte Stripe pleinement actif (KYC) = action Benji.
- **Login Google (OAuth)** : le code est branché (`signInWithOAuth({provider:'google'})`). Reste UNIQUEMENT = activer le provider Google dans **Supabase Auth** (client id/secret). Benji a déjà un projet Google Cloud (cf `YOUTUBE_CLIENT_ID`) réutilisable. ~5 min dashboard Supabase. Mon connecteur Google MCP (Gmail/Drive) ne crée PAS ce provider (système différent).
