...
1. **🥇 Onboarding dev à l'inscription = LA priorité.** Aujourd'hui un dev qui s'inscrit tombe sur un dashboard vide, sans flow guidé pour connecter son App Store Connect. Friction n°1 : sans ça il ne voit aucune donnée et part. À coder : **flow guidé pas-à-pas** pour saisir .p8, issuer id, key id, vendor number, app id + **validation live de la clé** (test d'appel ASC) + **1re synchro** + état « connecté/à connecter » sur l'accueil (bloc Setup / checklist). Gros effet rétention, isolé, faible risque.
   - ✅ **FAIT (18/06, loop, passe 12) :** **1re synchro AUTO après validation** de la clé ASC — la page App Store Connect redirige automatiquement vers `/dashboard` (qui déclenche le chargement des vraies données) ~2,5 s après un test de connexion réussi, au lieu du nudge manuel « Voir mes données ». Lien « Voir maintenant » conservé, timer nettoyé au démontage. tsc + eslint OK. **Reste onboarding :** le wizard plein-écran dédié (les liens vers les sous-pages restent dispersés) — à faire avec un retour Benji sur le compromis UX.
   - ✅ **FAIT (17/06, loop, passe 10) :** `SetupChecklist` transformé en **parcours guidé** : barre de progression + emphase « Étape suivante » (bouton primaire sur la 1re étape non faite, les autres discrètes) → une action claire à la fois pour un non-codeur, sans dupliquer les formulaires existants. tsc OK (lint : 0 erreur). **Reste onboarding (NON bloquant mais reporté) :** 1re synchro 100% programmatique après validation (vs nudge) + un wizard plein-écran dédié si besoin un jour.
- ✅ **FAIT (17/06, loop, passe 9) :** doc Connexions enrichie — usage funnel (`.appolynScreen` / `Appolyn.screen`) + **liste exacte des données collectées par le SDK pour la nutrition label App Privacy** (Identifiants/Achats/Usage/Diagnostics, anonyme IDFV, pas d'IDFA/ATT) = compliance rendue nécessaire par « capture max ». tsc+eslint OK.
- ✅ **FAIT (17/06, loop, passe 6) :** micro-aides « où trouver » sous Key ID + Issuer ID (pour non-codeurs) + **bandeau post-validation** « Connexion réussie → Voir mes données » sur la page App Store Connect (guide vers la 1re synchro). tsc+eslint OK. **Reste onboarding :** wizard pas-à-pas multi-étapes unifié (encore des liens dispersés vers les sous-pages) + déclenchement programmatique d'une 1re synchro automatique après validation (aujourd'hui = nudge manuel vers /dashboard).
   - ✅ **FAIT (17/06, loop) :** 4e étape **« Branche le SDK dans ton app »** ajoutée au `SetupChecklist` (accueil), avec détection `hasSdk` réelle (≥1 `sdk_clients` sur les apps du user), CTA vers Réglages → Connexions, compteur dynamique n/N, et `setupComplete` inclut désormais le SDK. Typecheck + lint OK (fichier propre). **Reste sur l'onboarding :** wizard pas-à-pas guidé (vs liens dispersés), déclenchement auto d'une 1re synchro après validation de la clé, micro-aides « où trouver Key ID / Issuer / Vendor » pour non-codeurs.
...
2. **Centre de notifications in-app** : panneau (cloche topbar) qui agrège les notices/alertes (les notices unitaires existent déjà). Additif, isolé, faible risque.
   - ✅ **FAIT (18/06, loop) :** `NotificationsBell` (composant dédié) remplace le placeholder « Aucune notification ». Agrège les VRAIES alertes actionnables dérivées de l'état réel du compte (mêmes signaux que le SetupChecklist : ASC connecté `asc_credentials`, app ajoutée, App ID renseigné, SDK branché `sdk_clients`), avec **badge de compte** sur la cloche + liens directs vers la bonne page. Quand tout est en place → « Tout est connecté » (jamais de faux « aucune notification »). Aucune donnée inventée. tsc + eslint OK. **Reste (évolutif) :** brancher d'autres notices réelles quand elles existeront (échec de synchro, nouveaux avis…).
...
## 🔁 Loop 19/06 (commits LOCAUX, non poussés)

- ✅ **Lint = 0 erreur** : 2 apostrophes JSX non échappées (reviews + copilot) corrigées. Restent 3 warnings INTENTIONNELS et corrects (audités) : 2 `useEffect` mount-only + 1 `<img>` 28px icône distante avec fallback `onError` (passer à `next/image` serait pire ici). Ne pas y retoucher.
- ✅ **Cohérence états vides** : `reviews` redéfinissait un `<Guide>` maison et `keywords` un `<EmptyState>` local → 3 traitements visuels différents. Migrés vers le `EmptyState` partagé (`components/dashboard/shell.tsx`, même pattern qu'analytics). -2 composants dupliqués, look + copy identiques. tsc + lint + build verts.
- 📌 **Vérifié, NE PAS reconstruire** : l'instantané (cache de session) est DÉJÀ branché sur toutes les pages data (analytics/keywords/competitors/reviews/clients via `lib/cache.ts`, metadata via cache local lignes 35-40). Les états vides « connecte ASC d'abord » existent déjà partout. Onboarding quasi complet. Le produit est mature : viser polish/cohérence/robustesse vérifiés, pas des features risquées (0 client → ne pas creuser le déséquilibre).
- ✅ **Cohérence états vides COMPLÈTE** : `metadata` (no-apps + no-creds) migré au `EmptyState` partagé (le « no apps » était nu, sans icône ni CTA → maintenant icône + action). `clients` laissé tel quel (état vide DANS une carte avec en-tête de tableau + bouton modale = contexte différent, exception justifiée). tsc + lint + build verts.
- ✅ **Chemin mort corrigé : Screenshots** — le hub Store (`store/page.tsx`) listait « Screenshots » en **« Bientôt »** alors que `ScreenshotsManager` (309 l.) est COMPLET (lecture captures ASC + traduction auto de l'accroche par langue + rendu + publication 1 clic + auto create-version). Promu en outil disponible (lien `/dashboard/screenshots`) **+ ajouté à la sidebar** (groupe Store Optimization, à côté de App Store Page/Keywords). Sous-vendait le produit. « Localization » reste honnêtement en « Bientôt » (pas de page dédiée). ⚠️ Effet de bord corrigé : la page autonome `/dashboard/screenshots` sans app connectée montrait « Aucun screenshot trouvé · ajoute des screenshots » (trompeur) → garde `!ascAppId` ajoutée dans le manager (« Connecte App Store Connect »), sans impact dans metadata (déjà gardé).
- ✅ **Onboarding ASC en 1 clic (principe « tout automatique »)** : sur la page `settings/app-store-connect`, « Enregistrer » déclenche maintenant `validate-credentials` juste après la sauvegarde (avant : 2 étapes Enregistrer + Tester, et tester avant d'enregistrer renvoyait un « invalide » trompeur). 1 clic = sauve + teste + redirige vers `/dashboard` si valide. Bouton « Tester » conservé pour re-tester une clé déjà en place.
- ✅ **Ajout d'app en 1 clic depuis ASC (« tout automatique », zéro saisie)** : l'action edge `list-apps` existait (déployée) mais n'était PAS branchée côté UI → le dev tapait nom + bundle + App ID à la main. `AddAppDialog` charge maintenant ses vraies apps ASC à l'ouverture : il clique → tout auto-rempli (nom/bundle/`asc_app_id`), apps déjà ajoutées grisées, repli « saisir manuellement » (Android / hors ASC / pas de clé). Pile dans le cadrage non-codeurs.
- ✅ **Cohérence états vides TERMINÉE** : `settings/apps` (dernier état vide plein-page inline) migré au `EmptyState` partagé. Récap : reviews, keywords, metadata, analytics, settings/apps utilisent tous le composant partagé ; `clients` reste l'exception justifiée (état vide DANS une carte + bouton modale).
- 🔜 **Prochain levier visé (rendu pro) :** afficher les **vraies icônes d'app** (au lieu du picto Smartphone générique) dans `settings/apps` + le sélecteur d'app de la topbar, via lookup iTunes (`/api/itunes`) par bundle_id/asc_app_id. Le composant icône+fallback existe déjà (`AppIconSmall` dans keywords) → réutilisable.
...
- ✅ **EN COURS (17/06, loop) :** la page Analytics avait DÉJÀ beaucoup de RevenueCat (KPIs MRR/ARR/activeSubs/renewalRate + bloc Abonnements actif/nouveau/perdu/renouvellement, via rapports Apple ASC). **Ajouté ce passage : bloc « Revenu par produit »** dérivé des events StoreKit du SDK (`sdk_events`), groupé par produit+prix → **l'A/B testing de prix (2,99 vs 4,99) ressort automatiquement** en lignes distinctes, sans config (exactement la demande Benji). Style existant réutilisé (calqué sur « Revenu par pays »), ajout au catalogue de blocs + merge de layout pour qu'il apparaisse chez les users existants. tsc+eslint OK. **Reste (types manquants) :** MRR movement (new/expansion/contraction/churned) — besoin de l'historique d'état d'abo, plutôt via rapports ASC ; refunds/refund rate — le SDK ignore les transactions révoquées (revocationDate), il faudrait les capter à part. ✅ **Essais + conversion d'essai FAITS (17/06, passe 8)** : SDK émet `trial_start` (détection offre d'intro gratuite via `tx.offer`/`offerType`, swift build OK) ; web = KPIs « Essais » + « Conversion d'essai » (idfv avec trial_start puis achat payant), style existant, tsc+eslint OK. ✅ **ARPU / ARPPU / LTV réalisée FAITS (17/06, passe 7)** : 3 KPIs dérivés des agrégats clients SDK (`sdk_clients` : revenu cumulé + payeurs), ajoutés au catalogue + KPIs par défaut, style existant, tsc+eslint OK.
...
- ✅ **EN COURS (17/06, loop) :** bout-en-bout posé. **SDK** : API `Appolyn.screen("Welcome")` (UIKit) + modifier SwiftUI `.appolynScreen("welcome")` (1 ligne) émettant un event `screen_view` (swift build OK). **Web** : nouveau bloc « Entonnoir d'onboarding » dans Analytics, dérivé des `screen_view` — distinct utilisateurs (idfv) par écran, **ordonné automatiquement par 1re apparition**, décrochage % calculé, barre + compteur dans le style existant. **Auto-adaptatif** : un nouvel écran instrumenté apparaît tout seul, sans config. tsc+eslint OK. **Reste :** l'**autocapture zéro-code UIKit (swizzling viewDidAppear)** — non fait ce passage car la branche `#if canImport(UIKit)` n'est PAS compilée par `swift build` sur macOS (donc invérifiable ici) et le swizzling est crash-prone : à faire dans un passage validable sur device.
...
- ✅ **FAIT (18/06, loop, passe 13) :** SDK v1.2.0 (Package + public synchronisés, identiques) — ajout de signaux techniques **Foundation-only, thread-safe, privacy-safe** (donc vérifiables par `swift build`, sans UIKit) : `timezone_offset`, `preferred_languages` (5 max), `physical_memory_mb`, `processor_count`, `disk_total_mb`, `disk_free_mb`, `is_jailbroken` (best-effort PASSIF par existence de fichiers, aucune écriture hors sandbox → rien à déclencher en App Review). Doc « données collectées » (page Connexions) mise à jour (Diagnostics : + mémoire/stockage, langues, fuseau). Swift build OK, tsc + eslint OK. **Reste (device-only, besoin UIKit/main-thread, non vérifiable ici) :** dark_mode, accessibilité (VoiceOver/Reduce Motion/Bold Text), taille de texte, type de connexion (NWPathMonitor), cycle de vie foreground/background + autocapture `screen_view` (swizzling).
- ✅ **FAIT (17/06, loop) :** (1) **ingest capture-max sans migration** — chaque event stocke désormais TOUT le contexte technique dans `properties._ctx` (denylist des secrets/colonnes), donc tout nouveau champ SDK est capté automatiquement, zéro changement de schéma ; (2) **SDK enrichi** (Package + public synchronisés, v1.1.0) avec signaux thread-safe : `session_count`, `seconds_since_install`, `is_update`, `previous_app_version`. Swift Package **build OK**, web **tsc + eslint OK**.
- **Déjà capté** : idfv, device_model, os, app_version/build, bundle_id, locale, language, region, timezone, install_date, screen w/h, device_class, low_power, is_simulator, ASA token, achats StoreKit (product_id + prix + devise), source (`setSource`), session_count, seconds_since_install, is_update, previous_app_version, + (v1.2.0) timezone_offset, preferred_languages, physical_memory_mb, processor_count, disk_total/free_mb, is_jailbroken, et tout le contexte dans `_ctx`.
...
- ~~**Stripe (abonnement Appolyn)**~~ ✅ **RÉSOLU & VÉRIFIÉ LIVE (19/06, API Stripe directe)** : RIEN à faire, tout existe déjà en compte **LIVE** : compte **activé** (`charges_enabled=true`, `details_submitted=true`, EUR/FR) ; 3 prix avec les bonnes lookup keys (`appolyn_monthly` 20€/mois, `appolyn_annual` 200€/an, `appolyn_ghost` 3€/mois) ; coupon `appolyn_retention_50_3m` (-50% × 3 mois) ; **webhook** `https://appolyn.vercel.app/api/billing/webhook` actif avec les 4 events gérés (checkout.session.completed + subscription created/updated/deleted) ; clés en Vercel prod. La facturation est **opérationnelle de bout en bout**. (Note historique : la mention « bloqué sur les clés/KYC de Benji » était périmée.)
- ~~**Login Google (OAuth)**~~ ✅ **RÉSOLU & VÉRIFIÉ LIVE (19/06, API Supabase)** : provider Google **activé** dans Supabase Auth (`external_google_enabled=true`, client_id présent), `site_url=https://appolyn.vercel.app`, redirect URLs OK. Le bouton « Continuer avec Google » fonctionne. Rien à faire.
