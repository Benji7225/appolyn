...
- ✅ **EN COURS (17/06, loop) :** la page Analytics avait DÉJÀ beaucoup de RevenueCat (KPIs MRR/ARR/activeSubs/renewalRate + bloc Abonnements actif/nouveau/perdu/renouvellement, via rapports Apple ASC). **Ajouté ce passage : bloc « Revenu par produit »** dérivé des events StoreKit du SDK (`sdk_events`), groupé par produit+prix → **l'A/B testing de prix (2,99 vs 4,99) ressort automatiquement** en lignes distinctes, sans config (exactement la demande Benji). Style existant réutilisé (calqué sur « Revenu par pays »), ajout au catalogue de blocs + merge de layout pour qu'il apparaisse chez les users existants. tsc+eslint OK. **Reste (types manquants) :** Active Trials + conversion d'essai (besoin event `trial_start` → enrichir SDK via StoreKit offer.type), MRR movement (new/expansion/contraction/churned), refunds/refund rate. ✅ **ARPU / ARPPU / LTV réalisée FAITS (17/06, passe 7)** : 3 KPIs dérivés des agrégats clients SDK (`sdk_clients` : revenu cumulé + payeurs), ajoutés au catalogue + KPIs par défaut, style existant, tsc+eslint OK.
