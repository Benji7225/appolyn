...
export default function HealthPage() {
  ...
  const compute = useCallback(async () => {
    ...
    const next: Pillar[] = [
      { key: 'launch', label: 'Préparation au lancement', icon: Rocket, weight: 25, score: launch, detail: 'Avance ta checklist guidée de lancement.', href: '/dashboard/launch', cta: 'Ouvrir la checklist' },
      { key: 'coverage', label: 'Couverture des langues', icon: Globe, weight: 25, score: coverage, detail: ascAppId ? 'Ajoute des langues pour toucher plus de marchés.' : 'Connecte App Store Connect pour mesurer.', href: '/dashboard/localization', cta: 'Voir la couverture' },
      { key: 'reputation', label: 'Réputation (note)', icon: Star, weight: 30, score: rating, detail: ascAppId ? 'Réponds aux avis pour soigner ta note.' : 'Connecte App Store Connect pour mesurer.', href: '/dashboard/reviews', cta: 'Gérer les avis' },
      { key: 'tracking', label: 'Suivi & veille', icon: Swords, weight: 20, score: tracking, detail: 'Suis des mots-clés et des concurrents pour piloter ton ASO.', href: '/dashboard/competitors', cta: 'Ajouter des concurrents' },
    ];
    setPillars(next);
