...
1. ✅ **FAIT+DÉPLOYÉ (20/06) — FUSION Localisation** = App Store Page + Localisation + Screenshots en UNE page `/app/localization` (textes par langue + mots-clés + description + texte promo + note ASO + screenshots via ScreenshotsManager + jauge de couverture % + copy « marché à conquérir »). Pages doublons supprimées, redirects `/app/metadata`+`/app/screenshots` → `/app/localization`. **RESTE (raffinement) :** mettre les screenshots DANS la modale par langue (au clic sur une langue, voir SES screenshots dédiés) — nécessite de vérifier/étendre le fetch screenshots par locale dans l'edge `asc-proxy` (action `get-screenshots` renvoie aujourd'hui la localisation primaire).
